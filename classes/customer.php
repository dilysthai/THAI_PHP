	...
	class customer
	{
		...
		public function insert_customers($data){
			...
			if($name=="" || $email=="" || $address==""   || $phone =="" || $password ==""){
				...
				if($result_check){
					$alert = "<span class='error'>Email Tồn Tại ! Vui Lòng Nhập Email Khác</span>";
					return $alert;
				}else{
					$query = "INSERT INTO tbl_customer(name,email,address,phone,password) VALUES('$name','$email','$address','$phone','$password')";
					$result = $this->db->insert($query);
					if($result){
						$alert = "<span class='success'>Đăng Kí Tài Khoản Thành Công</span>";
						return $alert;
					}else{
						$alert = "<span class='error'>Đăng Kí Tài Khoản Thất Bại</span>";
						return $alert;
					}
				}
			}
			...
			if($email=='' || $password==''){
				...
				if($result_check){
					...
					Session::set('customer_name',$value['name']);
					 $alert = header('Location:index.php'); // "<span class='success'>Đăng Nhập Thành Công -> <a href='index.php'>Mua Sắm Sản Phẩm</a></span>";
						return $alert;
				}else{
					$alert = "<span class='error'>Email or Password không đúng!</span>";
					return $alert;
				}
			}
		}
		...
		public function update_customers($data, $id){
			...
			if($name=="" ||  $email=="" || $address=="" || $phone ==""){
				...
				$result = $this->db->insert($query);
				if($result){
						$alert = "<span class='success'>Updated Thông Tin Thành Công!</span>";
						return $alert;
				}else{
						$alert = "<span class='error'>Updated Thông Tin Thất Bại!</span>";
						return $alert;
				}
